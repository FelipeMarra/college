...
{
	bool melhoria = TRUE;
	double bestFO = calcula_fo(s, n, p, w, b);
	double loopFO;
	while (melhoria)
	{
		melhoria = FALSE;
		...
		for (int i = 0; i < n; i++)
		{
			//Troca bit para gerar vizinho
			troca_bit(s, i);
			loopFO = calcula_fo(s, n, p, w, b);
			//Se a função é melhor break
			if (loopFO > bestFO)
			{
				melhoria = TRUE;
				bestFO = loopFO;
			...
			//Se não volta para a solução anterior
			else
			{
				troca_bit(s, i);
			}
...
void busca_local_melhor_aprimorante(int n, int *s, double *p, double *w, double b)
{
	...
	while (melhoria)
	{
		...
		for (int i = 0; i < n; i++)
		{
			...
			troca_bit(s, i);
			loopFO = calcula_fo(s, n, p, w, b);
			//Se essa solução é maior que
			if (loopFO > loopMaxFO)
			{
				loopMaxFO = loopFO;
				maxFOBit = i;
			}
			//Volta solução corrente para o estado inicial
			troca_bit(s, i);
		}
		if (loopMaxFO > bestFO)
		{
			troca_bit(s, maxFOBit);
			melhoria = TRUE;
		}
	}
}
...
void busca_local_randomica(int n, int *s, double *p, double *w, double b, int iter_max)
{
	...
	for (size_t i = 0; i < count; i++)
	{
		...
		{
			//Volta solução pro estado anterior
			troca_bit(s, randomBit);
		}
	}
}