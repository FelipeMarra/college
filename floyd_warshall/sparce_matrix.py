...
class SparceMatrix:
    ...
    def validate(self, origin, destiny=None):
        ...
        if not destiny in self.edge_map[origin]:
            print("this destiny is not there")
            return False
    ...
    def remove_vertex(self, origin: int, destiny: int):
        if not self.validate(origin, destiny):
            return
    ...
    def vertex_degree(self, origin: int):
        if not self.validate(origin):
            return

        return len(self.edge_map[origin])

    def edge_weight(self, origin: int, destiny: int):
        if not origin in self.edge_map:
            return float('inf')
        ...
        return self.edge_map[origin][destiny]["weight"]
    ...
    def print(self):
        for origin in self.edge_map:
            print("Vertex", origin, "goes to:")
            for data in self.edge_map[origin].values():
                destiny, weight = data.values()
                print("=> Vertex", destiny, "with weight", weight)
            print()
