	...
	do
	{
		...
		case 3: /* Metaheuristicas */
			...
			case 8: /* AG */
				...
				break;
			case 9: /* AC */
				AC(n, s, p, w, b, 10, 10, 0.5, 1, 5, 0.9, 20);
...
void VNS(int n, int *s, double *p, double *w, double b, int iter_max)
{
	...
	while (iter < iter_max)
	{
		...
		while (k <= 2)
		{
			...
			switch (k)
			{ //Gere um vizinho qualquer na vizinhanca corrente
			case 1:
				vizqqN1(n, sl, p, w, b);
				break;
	...
	while (iter < iter_max)
	{
		...
		for (int k = 0; k < numF; k++)
		{
			...
			for (int i = 0; i < n; i++)
			{
				...
				if (pop[k][i] == 1)
				{
					deltaF[i] += profit[i] * (1 / (1 + (fo_star - fo_pop[k]) / fo_star));
				}
			}
		}
		...
		// Atualiza feromonio
		for (int i = 0; i < n; i++)
			feromonio[i] = feromonio[i] * fator_evap + deltaF[i];
		//getchar();
	}
